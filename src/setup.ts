...
export async function setup(config?: ProxyOptions) {
  ...
  config.discord = isTrue(await configQuestion('Do you want to use the discord bot?', `${!!config.discord??!!allDefaults.discord}`))
    ...
    : null;
  console.log('Now to the extraOptions. (more detailed explanations to them available at https://github.com/rob9315/2b2wts/blob/master/README.md)');
  config.extra = {
  ...
  //returnDiff(config, new ProxyOptions());
