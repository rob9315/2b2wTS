...
const configQuestion = async (questionString: string, prevOption: string, blankStringValid?: boolean, stars?: number | boolean) => {
  let answer = await util.question(`${questionString} [${!!stars ? starString(prevOption, typeof stars === 'number' ? stars : undefined) : prevOption}]\n> `);
...
export async function setup(config?: ProxyOptions) {
  config = merge(new ProxyOptions(), config);
  ...
  config.webserver = isTrue(await configQuestion('Do you want to host the web-interface?', `${!!config.webserver ?? true}`))
    ...
    : null;
  let discord = { token: await configQuestion('Enter your discord bot Token if you want to use a discord bot. (more information on how to get one at https://github.com/rob9315/2b2wts/blob/master/README.md)', config.discord?.token ?? '', undefined, 15) };
  config.discord = undefinedIfNone(discord.token)
    ? {
        token: discord.token,
        status: isTrue(await configQuestion('Do you want the status of the discord bot to be set to the current queue progress?', `${config.discord ? config.discord.status : true}`)),
...
function returnDiff(userConf: { [key: string]: any } = {}, defaultConf: { [key: string]: any } | null = {}) {
  let diff: { [key: string]: any } = merge(defaultConf, userConf);
